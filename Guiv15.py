...
#run runArm() with the value of 1, if runArm has 1 as the variable passed
#and it passes the validation checks sortfunc will run. if sortfunc will
#run and has 1 as the variable passed, then Canvas solution will run instead of
#the os system command for rospub publisher
...
def CanvasSolution():
    ...
    MainCanvas = Canvas(window,scrollregion=(0, 0, 1000, 6000), yscrollcommand=scr.set)
...
def CanvasSetup():
    ...
    graphInt = Canvas(window, width=400, height=250)
...
def makeCanvasStart(GraphInt):
    ...
    numberAndColor = [(6, "red"), (5, "green"), (4, "red"), (3, "yellow"), (2, "green"), (1, "lightblue")]
    ...
    if len(cOne) > 0:
        for i in range(len(cOne)):
            height = baseYAxis
            GraphInt.create_rectangle(70, heightEquation(baseYAxis, i), 100, heightEquation(baseYAxis, i) + 30, fill=cOne[i][1])
            GraphInt.create_text(85,heightEquation(baseYAxis, i) + 15,text=cOne[i][0])
    if len(cTwo) > 0:
        for i in range(len(cTwo)):
            GraphInt.create_rectangle(130, heightEquation(baseYAxis, i), 160, heightEquation(baseYAxis, i) + 30, fill=cTwo[i][1])
            GraphInt.create_text(145,heightEquation(baseYAxis, i) + 15,text=cTwo[i][0])
    if len(cThree) > 0:
        for i in range(len(cThree)):
            GraphInt.create_rectangle(190, heightEquation(baseYAxis, i), 220, heightEquation(baseYAxis, i) + 30, fill=cThree[i][1])
            GraphInt.create_text(205, heightEquation(baseYAxis, i) + 15,text=cThree[i][0])
    if len(cFour) > 0:
        for i in range(len(cFour)):
            GraphInt.create_rectangle(250, heightEquation(baseYAxis, i), 280, heightEquation(baseYAxis, i) + 30, fill=cFour[i][1])
            GraphInt.create_text(265,heightEquation(baseYAxis, i) + 15,text=cFour[i][0])

    # labels for platforms
    GraphInt.create_text( 35 ,230,text="platform:" )
    j = 7
    for i in range(4):
        GraphInt.create_text( (j * 10) + 15 ,230,text=str(i + 1) )
        j = j + 6
