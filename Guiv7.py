...
def runArm():
    errhandle = 0
    count = 0
    errString = "ERROR: please change the following inputs with values between 1 and 4 \n"
    for x in Spinboxes:
        temp = x.get()
        if int(temp) == 0:
            x['fg'] = "red"
...
def CanvasSolution():
    ...
    file = open("solution_verify.txt", "r")
    movesArrRaw = file.read().splitlines()
    file.close()
    i = 0
...
class alphaFrame:
    ...
    def __init__(self, Root):
        ...
        visSol.grid(row = 0, column = 6, columnspan = 1)
